...
test("Test datastore append and prepend on a vertex buffer dataStore, manually selecting which inputs to operate on, predefined layout with differing definitions throws if definitions for involved inputs are different", () => {
  const gl = new GL(fakeCanvas);
  ...
  let layoutInputs = {
    y: {name: 'y', size: 1, type: 'float'},
    x: {name: 'x', size: 3, type: 'float'}, // different!!
    w: {name: 'w', size: 3, type: 'float'}, // different!!
    z: {name: 'z', size: 3, type: 'float'}  // different!!
  };
  ...
  expect(() => dataset.appendData(data, layout , {inputsToAdd: ['x', 'y']})).toThrow("layout object does not have the same input definitions as this dataset");
