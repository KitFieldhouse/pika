...
test("Test return values of gl.repeat variants", () =>{
    ...
    expect(GL.repeat("y", "x")).toEqual({[isLayoutObj]: true, arguments: ['y', 'x'], repeatType: 'start', opts: defaultRepeatOpts});
    expect(GL.endRepeat("y", "x")).toEqual({[isLayoutObj]: true, arguments: ['y', 'x'], repeatType: 'end', opts: defaultRepeatOpts});
    expect(GL.centerRepeat("y", "x")).toEqual({[isLayoutObj]: true, arguments: ['y', 'x'], repeatType: 'center', opts: defaultRepeatOpts});

    // with optional param

    expect(GL.repeat({size: 100} , "y", "x")).toEqual({[isLayoutObj]: true, arguments: ['y', 'x'], repeatType: 'start', opts: {size: 100}});
    expect(GL.endRepeat({size: 100} ,"y", "x")).toEqual({[isLayoutObj]: true, arguments: ['y', 'x'], repeatType: 'end', opts: {size: 100}});
    expect(GL.centerRepeat({size: 100} , "y", "x")).toEqual({[isLayoutObj]: true, arguments: ['y', 'x'], repeatType: 'center', opts: {size: 100}});
